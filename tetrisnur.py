...
class TetrisAnimation(Scene):
    ...
    def update_preview(self):
        """Update the preview display with next pieces"""
        # Remove old preview pieces
        for piece in self.preview_pieces:
            self.remove(piece)
        self.preview_pieces.clear()
        
        # Create and position new preview pieces
        for i, piece_type in enumerate(self.piece_queue):
            preview_piece = self.create_piece(piece_type, 0, 0)
            preview_piece.scale(0.7)  # Make preview pieces slightly smaller
            preview_piece.move_to(self.preview_positions[i])
            self.preview_pieces.append(preview_piece)
            self.add(preview_piece)

    def play_round(self):
        """Updated play round with better error handling and continuous play"""
        end_time = time.time() + 2400  # 40 minutes runtime
        game_over = False
        ...
        while time.time() < end_time and not game_over:
            try:
                ...
                # Reset board if it gets too high
                ...
                if self.check_collision(piece):
                    # Try to recover instead of ending game
                    self.reset_top_portion()
                    continue
    ...
    def check_board_height(self):
        ...
        return filled_count > (6 * self.GRID_WIDTH * 0.3)  # 30% threshold

    def reset_top_portion(self):
        """More aggressive reset that maintains playability"""
        clear_height = self.GRID_HEIGHT // 2  # Clear top half
        
        # Keep bottom half intact
        self.grid = np.vstack([
            np.zeros((clear_height, self.GRID_WIDTH)),
            self.grid[clear_height:]
        ])
        
        blocks_to_remove = VGroup()
        for block in self.placed_blocks:
            pos = self.get_grid_position(block)
            if pos[0] < clear_height:
                blocks_to_remove.add(block)
        
        self.play(FadeOut(blocks_to_remove), run_time=0.03)
    ...
    def check_lines(self):
        ...
        if lines_to_clear:
            # Keep original score value but clear lines faster
            self.score += len(lines_to_clear) * 100
            
            # Update scoreboard
            self.scoreboard.become(
                Text(
                    f"SCORE: {self.score}",
                    color=WHITE,
                    weight=BOLD
            ...
            for line in lines_to_clear:
                ...
                self.grid = np.vstack([np.zeros(self.GRID_WIDTH), self.grid])
                
                # Make line clearing twice as fast
                self.play(
                    FadeOut(blocks_to_remove),
                    blocks_to_move.animate.shift(DOWN * self.BLOCK_SIZE),
                    run_time=0.05  # Changed from 0.1 to 0.05
                )
