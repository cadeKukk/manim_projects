...
class AmbientTilingChanges(CubesAsHexagonTiling):
    ...
    def construct(self):
        # Initialize with fewer cubes
        self.pre_populate()  # This fills half the spaces
        
        # Try to add new cubes, but handle when space runs out
        attempts = 5
        for _ in range(attempts):
            new_cube = self.random_new_cube()
            if new_cube is None:  # No more space available
                print("No more space for new cubes")
                break
                
            self.animate_in_with_rotation(new_cube, run_time=1)  # Reduced from default
            old_cube = self.get_random_cube_from_stack()
            if old_cube is not None and old_cube is not new_cube:
                self.animate_out_with_rotation(old_cube, run_time=1)  # Reduced from default
            
            self.wait(0.2)  # Reduced wait time between iterations
...
class MongesTheorem(InteractiveScene):
    ...
    def get_cones(self, circles, angle=90 * DEGREES):
        ...
        for circle in circles:
            ...
            # Create cone using ParametricSurface
